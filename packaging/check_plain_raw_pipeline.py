...
def main() -> None:
    ...
    if int(rgb.max()) == 0 or float(rgb.mean()) < 1.0:
        raise RuntimeError("plain RAW temporary-DNG pipeline returned a black RGB image")

    print(f"plain RAW pipeline: ok (temp={tempfile.gettempdir()}, mean={rgb.mean():.2f})")
